...
from ryu.ofproto import inet # Para IPPROTO_UDP, IPPROTO_ICMP
...
class DijkstraController(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        self.logger.info(f"DEBUG: PacketIn recibido en switch={dpid} in_port={in_port} eth_type={eth.ethertype:04x}")
        ...
        if is_dst_multicast:
            ...
            if _ipv4 and (_ipv4.dst.startswith('224.') or _ipv4.dst.startswith('239.')):
                self.logger.info(f"DEBUG: Tráfico IP Multicast detectado: {_ipv4.dst} de {src_mac} a {dst_mac} en switch {dpid} puerto {in_port}. Protocolo IP: {_ipv4.proto}")
